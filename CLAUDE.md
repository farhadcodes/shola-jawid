...
| **Persian Calendar** *(`persian-calendar`, approved 2026-08-06)* | Site-wide Gregorian→Jalali calendar conversion + Persian-digit dates (`get_the_date()`, `date_i18n()`, `wp_date()`, etc.) | Full Jalali calendar math (leap-year rules, month lengths) is a well-defined but nontrivial algorithm better maintained by a dedicated, widely-used package (100+ RTL-locale installs) than hand-rolled per template. Evaluated against ParsiDate first (100,000+ installs) — rejected because its date-conversion hooks are hardcoded to `get_locale() === 'fa_IR'` only, excluding this project's `fa_AF` (Afghanistan Dari) locale entirely; Persian Calendar gates on `is_rtl()` instead, which covers `fa_AF`. **Caution:** Persian Calendar's `regional_settings` option (on by default) force-overwrites `timezone_string` to `Asia/Tehran` on init — wrong for this project, disabled immediately after install; verify this setting stays off after any plugin update. Full evaluation and incident logged in `docs/CHANGELOG.md` 2026-08-06. |
| **FileBird** *(`filebird`, approved 2026-09-05)* | Folder/subfolder organization inside the Media Library (create, rename, delete, and move PDFs/images/videos into folders) — requested by Farhad as the media library grows and becomes hard to manage as one flat pile. | WordPress core has no folder concept for the Media Library at all. A real, drag-and-drop folder UI inside the existing Grid view means extending WordPress's own internal Backbone.js media-library JavaScript — a nontrivial, ongoing-maintenance surface (that internal JS isn't a stable public API and can shift under future WP core updates), the same category of problem CF7 and Persian Calendar were already whitelisted for rather than reinventing. Evaluated against building a fully custom taxonomy-based version (List-view-only filtering, no grid drag-and-drop) — rejected once a genuinely free option was confirmed, since the client specifically wants the visual folder/drag-and-drop experience, not just a filter dropdown. Verified directly on WordPress.org before approving: 200,000+ active installs, 4.7★ (1,121 reviews), last updated August 2026 — actively maintained, not abandoned; and confirmed the *free* version (not a crippled "lite" tier) includes unlimited folders/subfolders and full drag-and-drop, with the paid tier only adding unrelated features (posts/pages folders, page-builder integrations, ZIP export). Public WordPress.org plugin, unlike this project's own `shola-core` — install via **افزونه‌ها → افزودن افزونه** (Add New Plugin) → search "FileBird", not a manual zip upload. |
