 ...
 * shola_get_featured_image(), shola_get_english_month_abbr(), and
 * shola_get_gregorian_year() all operate on any $post generically, none
 * assume `issue`. Fields the calling sections show elsewhere but this
 * partial never has (issue number, publication term, author/source,
 * collection term, PDF link) stay out of it — same "no dek/byline"
 * anatomy for both types, not a redesign.
 *
 * @param array $args {
...
$date_label = shola_get_english_month_abbr( $card_post ) . ' ' . shola_to_persian_digits( shola_get_gregorian_year( $card_post ) );
?>
